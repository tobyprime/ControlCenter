...
test.describe('设备管理（TOB-337）', () => {
  test('新建设备 → 签发 token → 列表离线展示 → 编辑 → 重置 token → 删除', async ({ page }) => {
    await login(page)
    ...
    await page.screenshot({ path: `${EVIDENCE_DIR}/devices-token-dialog.png`, fullPage: true })
    ...
    await page.screenshot({ path: `${EVIDENCE_DIR}/devices-list-offline.png`, fullPage: true })
  ...
  test('375px 移动视口设备页响应式无横向溢出', async ({ page }) => {
    await login(page)
    await page.setViewportSize({ width: 375, height: 720 })
    await page.goto('/devices')
    await expect(page.getByRole('heading', { name: '设备管理' })).toBeVisible()
    await expectNoHorizontalOverflow(page, '设备页 375px')
    await page.screenshot({ path: `${EVIDENCE_DIR}/devices-mobile-375.png`, fullPage: true })
  })
})
