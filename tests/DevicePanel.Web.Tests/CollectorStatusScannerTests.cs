...
/// <summary>push 采集器在线状态采样器测试：判定离线即写一次 online=false 样本并喂规则引擎；恢复由心跳链路写 true。</summary>
public class CollectorStatusScannerTests : IDisposable
{
    ...
    private readonly CollectorStatusScanner _scanner;
    ...
    public CollectorStatusScannerTests()
    {
        ...
        _scanner = new CollectorStatusScanner(
            _collectors, _metrics, _engine, _agentOptions, new AlertOptions(), _clock,
            Microsoft.Extensions.Logging.Abstractions.NullLogger<CollectorStatusScanner>.Instance);

        // push 采集器：关联 agent（扫描器只扫有 agent 的采集器）
        var agents = new AgentRegistry(_db.Factory, _clock);
        var agent = agents.Create("在线设备 agent", []).Agent;
        _collectorId = _collectors.Create("在线设备", ["type:device"], agent.Id).Id;
    ...
    public void Offline_Transition_Writes_Online_False_Sample_Once_And_Rule_Fires()
    {
        ...
        _collectors.Touch(_collectorId, _clock.GetUtcNow());
        _metrics.Insert(_collectorId, MetricKeys.Online, new MetricSample(_clock.GetUtcNow(), 1, "true"));

        // 掉线：超过 OfflineAfter（连续 2 个周期）未心跳
        _clock.Advance(TimeSpan.FromSeconds(61));
        _scanner.ScanOnce();

        var latest = _metrics.GetLatest(_collectorId, MetricKeys.Online);
        ...
        Assert.Equal("false", _metrics.GetLatest(_collectorId, MetricKeys.Online)!.ValueText);
    }

    [Fact]
    public void Online_Collector_Produces_No_Sample()
    {
        _collectors.Touch(_collectorId, _clock.GetUtcNow());

        _scanner.ScanOnce();

        Assert.Null(_metrics.GetLatest(_collectorId, MetricKeys.Online));
        Assert.Empty(_outbox.List());
    }

    [Fact]
    public void Never_Seen_Collector_Is_Skipped()
    {
        _rules.Create(null, MetricKeys.Online, StateMismatchRuleType.TypeIdValue, """{"expected":"true"}""", sustainSeconds: 0, repeatMinutes: 0, enabled: true);
    ...
    public void Recovery_Via_Heartbeat_True_Sample_Closes_Event()
    {
        ...
        _metrics.Insert(_collectorId, MetricKeys.Online, trueSample);
        _engine.OnSample(_collectorId, MetricKeys.Online, trueSample, recoveryAt);
        _clock.Advance(TimeSpan.FromSeconds(61));
        _scanner.ScanOnce();
    ...
    [Fact]
    public void Pull_Collectors_Are_Ignored()
    {
        // pull 采集器：无 agent 回连，不归心跳扫描（status 样本由面板侧轮询产出）
        var pullId = _collectors.Create("MC 服务", ["type:service"]).Id;
        _collectors.Touch(pullId, _clock.GetUtcNow().AddHours(-1));

        _scanner.ScanOnce();

        Assert.Null(_metrics.GetLatest(pullId, MetricKeys.Online));
    }
